...
export default function AgendaPage() {
  ...
  // Listagem de opções únicas para os filtros com base nos dados do mock
  const units = useMemo(() => {
    const allUnits = courseEvents.map(e => e.locationName);
    return ['all', ...Array.from(new Set(allUnits))];
  }, []);

  const categories = useMemo(() => {
    const allCats = courseEvents.map(e => e.category);
  ...
  }, [filterUnit, filterModality, filterShift, filterCategory]);
            ...
            {filteredEvents.length === 0 ? (
              <div className="bg-white rounded-2xl border border-slate-200/80 p-12 text-center space-y-4">
                ...
                <button
                  onClick={resetFilters}
                  type="button"
                  className="inline-flex items-center justify-center rounded-xl bg-primary px-6 py-3 text-sm font-bold text-white shadow-md shadow-primary/10 hover:bg-primary-hover transition-colors"
                >
                  Ver Todos os Cursos
                </button>
              </div>
